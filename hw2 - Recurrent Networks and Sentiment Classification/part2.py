...
class NetworkLstm(tnn.Module):
    ...
    def forward(self, input, length):
        ...
        """
        packed = tnn.utils.rnn.pack_padded_sequence(input=input, lengths=length)
...
class NetworkCnn(tnn.Module):
    ...
    def __init__(self):
        ...
        self.relu = tnn.ReLU()
        self.maxpool1 = tnn.MaxPool1d(kernel_size=4)
        self.maxpool2 = tnn.MaxPool1d(kernel_size=4)
    ...
    def forward(self, input, length):
        ...
        Create the forward pass through the network.
        """
        output = self.conv1(input.permute(0, 2, 1))
        output = self.relu(output)
        ...
        output = self.relu(output)
        maxpool3 = tnn.MaxPool1d(kernel_size=output.size(2))
        output = maxpool3(output)
...
def measures(outputs, labels):
    ...
    for i in range(outputs.size(0)):
        if outputs[i] >= 0 and int(labels[i]) == 1:
            tp += 1
        elif outputs[i] >= 0 and int(labels[i]) == 0:
            fp += 1
        elif outputs[i] < 0 and int(labels[i]) == 1:
            fn += 1
        elif outputs[i] < 0 and int(labels[i]) == 0:
            tn += 1
    return tp, tn, fp, fn
